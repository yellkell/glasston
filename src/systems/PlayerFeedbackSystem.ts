 ...
 * pointing toward where the shot came from, that quickly fades. Head-locked and
 * alpha-blended (no passthrough halo); it never moves you — purely a visual cue.
 *
 * The incoming direction is stored in world space (see fx/feedback); we rotate
 * it into head-local each frame so the glow tracks the real source even as you
 * turn your head.
 */
...
export class PlayerFeedbackSystem extends createSystem({}) {
  ...
  private drawGlow(theta: number, behind: boolean): void {
    ...
    g.addColorStop(0, 'rgba(235,40,45,0.85)');
    g.addColorStop(0.5, 'rgba(225,30,40,0.35)');
